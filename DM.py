...
def sorted_add(q, item):
	if q is None:
		...
		if temp.next is None:
			if item.relative_deadline >= temp.relative_deadline:
				temp.next = item
				return temp
			else:
				item.next = temp
			...
			if item.relative_deadline < temp.relative_deadline:
				...
				while temp.next is not None and item.relative_deadline >= temp.next.relative_deadline:
					temp = temp.next
				item.next = temp.next
				temp.next = item
	return q
